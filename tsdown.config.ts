/**
 * tsdown client-bundle build for dsh-anysearch-refs.
 *
 * Mirrors @deepseek-ai/dsh-better-sidebar's clientBundle() + makeCssPlugin():
 * the browser client is compiled from src/client/index.tsx into CJS closure
 * factories that register with window.__ModuleLoader__.load({ id, factory }),
 * and every CSS Module is compiled to a hashed class map AND injected as a
 * <style data-plugin-css="<id>/<file>"> tag at factory execution (so the
 * plugin's styles land in document.head like DSH's own client bundles do).
 *
 * This ONLY builds the two client bundles. The host half (src/index.ts,
 * src/host/*, src/shared/*) is still built by `tsc -p tsconfig.json` — that
 * tsconfig is untouched.
 *
 * - lib/client.js          -> id `dsh-anysearch-refs`        (bundle channel)
 * - lib/client-registry.js -> id `dsh-anysearch-refs/dsh-anysearch-refs`
 *                                                  (registry channel)
 */
...
function purityGatePlugin(): BuildPlugin {
  return {
    ...
    resolveId(source: string) {
      ...
      throw new Error(
        `client bundle purity: "${source}" is not a platform module (CLIENT_EXTERNALS) — cross-plugin value imports are forbidden; collaborate through cordis services`,
      )
    },
  }
}
...
* differing, so they cannot drift.
...
function clientBundle(pluginId: string, entryFile: string): UserConfig {
  return {
    ...
    // External wins for module-table entries; everything else inlines.
...
export default [
  // Host half: bundle src/index.ts into a single Node ESM entry. The
  ...
  // Bundle channel: id = package name (package.json `name`).
  clientBundle('dsh-anysearch-refs', 'client.js'),
  // Registry channel: id = manifest id (dsh.plugin.json `id`).
  clientBundle('dsh-anysearch-refs/dsh-anysearch-refs', 'client-registry.js'),
]
